# DrugClaw Testing Guide

This document describes how to test every feature of DrugClaw. It includes both automated tests (unit/integration) and manual black-box functional tests organized by user stories.
...
Since DrugClaw is a multi-platform bot with external dependencies (LLM APIs, Telegram/Discord/WhatsApp APIs, DuckDuckGo), many features require live interaction testing.

### Prerequisites

1. A working `drugclaw.config.yaml` file with valid credentials
...
|---|-----------|-------|----------|
| 1.1 | First launch without config | Run `drugclaw start` with no config.yaml present | Auto-launches setup wizard |
| 1.2 | Normal startup | Fill config.yaml, run `drugclaw start` | Logs: Database initialized / Memory manager initialized / Scheduler started |
...
| 1.5 | CLI help | `drugclaw help` | Full output: commands, features, config docs |
| 1.6 | CLI version | `drugclaw version` | Output: `drugclaw {VERSION}` |
| 1.7 | Unknown command | `drugclaw foobar` | "Unknown command: foobar" + help text |
| 1.8 | Setup wizard | `drugclaw setup` | TUI interactive guide, provider/model selection |
| 1.9 | MICROCLAW_CONFIG env var | `MICROCLAW_CONFIG=/tmp/test.yaml drugclaw start` | Loads config from specified path |
...
| # | User Story | Steps | Expected |
|---|-----------|-------|----------|
| 25.1 | Install service | `drugclaw gateway install` | macOS: LaunchAgent; Linux: systemd service |
| 25.2 | Start service | `drugclaw gateway start` | Service starts |
| 25.3 | Check status | `drugclaw gateway status` | Reports running state |
| 25.4 | View logs | `drugclaw gateway logs 50` | Shows last 50 log lines |
| 25.5 | Stop service | `drugclaw gateway stop` | Service stops |
| 25.6 | Uninstall service | `drugclaw gateway uninstall` | Service removed |
...
```sh
sqlite3 drugclaw.data/runtime/drugclaw.db
...
```sh
# Remove test files
rm -f /tmp/drugclaw_test.txt /tmp/todos.txt /tmp/test.txt /tmp/session_test.txt

# Cancel any remaining scheduled tasks via the bot, or:
sqlite3 drugclaw.data/runtime/drugclaw.db "UPDATE scheduled_tasks SET status='cancelled' WHERE status='active';"
```
