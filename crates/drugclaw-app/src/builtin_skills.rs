...
static GENOMICS_SKILLS_DIR: Dir<'_> = include_dir!("$CARGO_MANIFEST_DIR/../../skills/genomics");
static MEDICAL_SKILLS_DIR: Dir<'_> = include_dir!("$CARGO_MANIFEST_DIR/../../skills/medical");
static PHARMA_SKILLS_DIR: Dir<'_> = include_dir!("$CARGO_MANIFEST_DIR/../../skills/pharma");
static RESEARCH_SKILLS_DIR: Dir<'_> = include_dir!("$CARGO_MANIFEST_DIR/../../skills/research");
static SCIENCE_SKILLS_DIR: Dir<'_> = include_dir!("$CARGO_MANIFEST_DIR/../../skills/science");
...
pub fn ensure_builtin_skills(skills_root: &Path) -> std::io::Result<()> {
    ...
    for embedded in [
        ...
        copy_compatible_skills(embedded, skills_root)?;
    }
    Ok(())
}
...
fn copy_compatible_skills(embedded: &Dir<'_>, destination: &Path) -> std::io::Result<()> {
    for entry in embedded.entries() {
        ...
        tracing::debug!("Found bundled skill: {}", skill_name_str);
        ...
        if let Some(reason) = skill_skip_reason(&content) {
            tracing::debug!(
                "Skipping bundled skill '{}' on this host: {}",
                skill_name_str,
                reason
            );
            continue;
        ...
        let overwrite_managed = existing_skill_is_managed(&next_dest);
        if !next_dest.exists() {
            tracing::debug!(
                "Installing skill {} to {}",
                skill_name_str,
            ...
            copy_embedded_entries(skill_dir, &next_dest, true)?;
        } else {
            tracing::debug!(
            ...
            copy_embedded_entries(skill_dir, &next_dest, overwrite_managed)?;
        }
    }
    Ok(())
}
...
fn existing_skill_is_managed(skill_dir: &Path) -> bool {
    let skill_md = skill_dir.join("SKILL.md");
    let Ok(content) = std::fs::read_to_string(skill_md) else {
        return false;
    };
    parse_frontmatter(&content)
        .map(|fm| fm.source.eq_ignore_ascii_case("drugclaw"))
        .unwrap_or(false)
}

fn copy_embedded_entries(
    embedded: &Dir<'_>,
    destination: &Path,
    ...
    for entry in embedded.entries() {
        match entry {
            DirEntry::Dir(dir) => {
                ...
                if should_skip_embedded_name(name) {
                    continue;
                }
                ...
                copy_embedded_entries(dir, &next_dest, overwrite_existing)?;
            }
            DirEntry::File(file) => {
...
fn should_skip_embedded_name(name: &std::ffi::OsStr) -> bool {
    let value = name.to_string_lossy();
    if value == ".DS_Store" || value == "__pycache__" {
        return true;
    }
    matches!(
...
mod tests {
    ...
    #[test]
    fn test_ensure_builtin_skills_refreshes_managed_skill_files() {
        let root = temp_root();
        let skills_root = root.join("skills");
        let managed_skill = skills_root.join("bio-tools");
        std::fs::create_dir_all(&managed_skill).unwrap();
        let managed_file = managed_skill.join("SKILL.md");
        std::fs::write(
            &managed_file,
            r#"---
name: bio-tools
...
"#,
        ...
        let embedded = SCIENCE_SKILLS_DIR.get_dir("bio-tools").unwrap();
        let expected = String::from_utf8_lossy(
            get_file_with_relative_dir(embedded, "SKILL.md")
                .unwrap()
                .contents(),
        )
        .to_string();
        let actual = std::fs::read_to_string(managed_file).unwrap();
    ...
    #[test]
    fn test_ensure_builtin_skills_skips_generated_cache_files() {
        let root = temp_root();
        let skills_root = root.join("skills");
        ensure_builtin_skills(&skills_root).unwrap();

        assert!(!skills_root.join(".DS_Store").exists());
        assert!(!skills_root
            .join("chem-tools")
            .join("templates")
            .join("__pycache__")
        ...
        assert!(!skills_root
            .join("chem-tools")
            .join("templates")
            .join("admet_screen.cpython-312.pyc")
            .exists());
