...
1. Backup SQLite database (`drugclaw.db`).
2. Record current config (`drugclaw.config.yaml`).
...
2. Verify discovery with `drugclaw hooks list`.
3. Enable one-by-one with `drugclaw hooks enable <name>`.
...
- #195 `mcp: strip internal drugclaw keys from forwarded args`
- #192 `Journald`
- #191 `add flake.nix`
...
3. Restore pre-upgrade `drugclaw.db` backup.
4. Restore previous `drugclaw.config.yaml`.
