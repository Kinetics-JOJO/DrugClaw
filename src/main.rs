...
use drugclaw::config::Config;
use drugclaw::error::DrugClawError;
use drugclaw::{
...
enum MainCommand {
    ...
    Reembed,
    /// Upgrade DrugClaw to latest release
    Upgrade,
    /// Show version
    Version,
}
...
fn handle_upgrade_cli() -> anyhow::Result<()> {
    ...
    println!("Upgrade completed. Re-run `drugclaw version` to verify.");
    Ok(())
}
...
Usage:
  drugclaw web [password <value> | password-generate | password-clear]
...
fn handle_web_cli(action: Option<WebAction>) -> anyhow::Result<()> {
    ...
    if matches!(action, Some(WebAction::PasswordClear)) {
        ...
        println!(
            "State is now uninitialized. On next `drugclaw start`, default password bootstrap policy will apply."
        );
        return Ok(());
    }
...
async fn main() -> anyhow::Result<()> {
    ...
    let launch_mode = match cli.command {
        ...
        Some(MainCommand::Setup(setup_args)) => {
            if setup_args.enable_sandbox {
                ...
                if !setup_args.yes && !setup_args.quiet {
                    println!(
                        "Tip: run `drugclaw doctor sandbox` to verify container runtime and image readiness."
                    );
                }
            } else {
                let saved = setup::run_setup_wizard()?;
                if saved {
                    println!("Setup saved to drugclaw.config.yaml");
                } else {
        ...
        Some(MainCommand::Skill { args }) => {
            let config = Config::load()?;
            drugclaw::clawhub::cli::handle_skill_cli(&args, &config).await?;
            return Ok(());
        }
...
mod tests {
    ...
    fn unique_temp_dir() -> std::path::PathBuf {
        let dir = std::env::temp_dir().join(format!("drugclaw-main-test-{}", uuid::Uuid::new_v4()));
    ...
    fn cli_parses_global_config_option_for_start() {
        ...
        let config = cli
            ...
            .unwrap_or_default();
        assert_eq!(config, "api_test_drugclaw.config.yaml");
    ...
    fn apply_config_override_accepts_relative_path() {
        ...
        let cfg = base.join("api_test_drugclaw.config.yaml");
    ...
    fn migrate_legacy_runtime_layout_only_migrates_whitelisted_entries() {
        ...
        std::fs::write(souls_dir.join("bot.md"), "soul").expect("write soul file");
        std::fs::write(root.join("drugclaw.db"), "db").expect("write db file");

        migrate_legacy_runtime_layout(&root, Path::new(&runtime_dir));

        assert!(root.join("drugclaw.db").exists());
        assert!(!groups_dir.exists());
        assert!(!runtime_dir.join("drugclaw.db").exists());
