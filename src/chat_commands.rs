...
use drugclaw_core::llm_types::Message;
use drugclaw_storage::db::{call_blocking, Database};
use drugclaw_storage::usage::build_usage_report;
use drugclaw_tools::todo_store::clear_todos;
...
pub async fn handle_chat_command(
    ...
    if trimmed == "/start" {
        if let Some(id) = sender_id.map(str::trim).filter(|v| !v.is_empty()) {
            return Some(format!("Hello DrugClaw :) Your ID: {id}"));
        }
        return Some("Hello DrugClaw :)".to_string());
    }
