...
pub trait LlmProvider: Send + Sync {
    ...
    ) -> Result<MessagesResponse, DrugClawError>;
    ...
    async fn send_message_with_model(
        ...
        loop {
            ...
            if let Ok(err) = serde_json::from_str::<OaiErrorResponse>(&text) {
                return Err(DrugClawError::LlmApi(err.error.message));
            }
            return Err(DrugClawError::LlmApi(format!("HTTP {status}: {text}")));
        }
    }
...
fn parse_openai_codex_response_payload(text: &str) -> Result<OaiResponsesResponse, DrugClawError> {
    ...
    Err(DrugClawError::LlmApi(format!(
...
mod tests {
    ...
    fn test_resolve_openai_compat_base_defaults_openai_codex() {
        ...
        let temp = std::env::temp_dir().join(format!(
            "drugclaw-llm-codex-base-default-{}",
            std::time::SystemTime::now()
                .duration_since(std::time::UNIX_EPOCH)
                .unwrap_or_default()
                .as_nanos()
