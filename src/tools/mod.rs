...
use drugclaw_storage::db::Database;
pub use drugclaw_tools::runtime::{
...
use drugclaw_tools::runtime::{inject_auth_context, require_high_risk_approval};
use drugclaw_tools::sandbox::{ExtraMount, SandboxMode, SandboxRouter};
...
mod tests {
    ...
    async fn test_high_risk_tool_requires_explicit_approval_on_web() {
        ...
        let approved = registry
            .execute_with_auth(
                "bash",
                json!({"__drugclaw_high_risk_approved": true}),
                &auth,
            )
            .await;
    ...
    async fn test_dynamic_plugin_tool_executes_without_restart() {
        let root = std::env::temp_dir().join(format!("drugclaw_plugin_{}", uuid::Uuid::new_v4()));
