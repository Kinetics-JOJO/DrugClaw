...
pub async fn serve(
    ...
    local
        .run_until(async move {
            ...
            let agent = DrugClawAcpAgent::new(app_state, outbound_tx);
...
impl Agent for DrugClawAcpAgent {
    async fn initialize(&self, args: InitializeRequest) -> AcpResult<InitializeResponse> {
        Ok(InitializeResponse::new(args.protocol_version)
            .agent_info(
                Implementation::new("drugclaw", env!("CARGO_PKG_VERSION")).title("DrugClaw"),
            )
            .agent_capabilities(
    ...
    async fn prompt(&self, args: PromptRequest) -> AcpResult<PromptResponse> {
            ...
            handle_chat_command(&self.app_state, chat_id, ACP_CHANNEL, &prompt_text, None).await
        {
            ...
            self.store_message(chat_id, "drugclaw", command_reply, true)
                .await
                .map_err(to_acp_error)?;
        ...
        self.store_message(chat_id, "drugclaw", response, true)
            .await
            .map_err(to_acp_error)?;
...
fn default_mode_state() -> SessionModeState {
    SessionModeState::new(
        ...
        vec![SessionMode::new(ACP_MODE_ID, "Chat")
            .description("General-purpose DrugClaw chat mode.")],
    )
}
