...
use drugclaw_tools::sandbox::DEFAULT_SANDBOX_IMAGE;
pub use drugclaw_tools::sandbox::{SandboxBackend, SandboxConfig, SandboxMode, SecurityProfile};
pub use drugclaw_tools::types::WorkingDirIsolation;
use drugclaw_tools::web_content_validation::WebContentValidationConfig;
use drugclaw_tools::web_fetch::WebFetchUrlValidationConfig;
...
impl Config {
    ...
    pub fn resolve_config_path() -> Result<Option<PathBuf>, DrugClawError> {
        ...
        if std::path::Path::new("./drugclaw.config.yaml").exists() {
            return Ok(Some(PathBuf::from("./drugclaw.config.yaml")));
        }
        if std::path::Path::new("./drugclaw.config.yml").exists() {
            return Ok(Some(PathBuf::from("./drugclaw.config.yml")));
        }
        Ok(None)
    }
    ...
    pub fn load() -> Result<Self, DrugClawError> {
        let yaml_path = Self::resolve_config_path()?;

        if let Some(path) = yaml_path {
            let path_str = path.to_string_lossy().to_string();
            let content = std::fs::read_to_string(&path)
                .map_err(|e| DrugClawError::Config(format!("Failed to read {path_str}: {e}")))?;
...
`web_auth_token` / `channels.web.auth_token` are ignored; migrate to operator password + API keys.",
            ...
            let mut config: Config = serde_yaml::from_str(&content)
                .map_err(|e| DrugClawError::Config(format!("Failed to parse {path_str}: {e}")))?;
            config.post_deserialize()?;
            return Ok(config);
        }

        // No config file found at all
        Err(DrugClawError::Config(
            "No drugclaw.config.yaml found. Run `drugclaw setup` to create one.".into(),
        ))
    }

    /// Apply post-deserialization normalization and validation.
    pub(crate) fn post_deserialize(&mut self) -> Result<(), DrugClawError> {
        ...
        if tz_raw.eq_ignore_ascii_case("auto") {
            ...
            tz_raw
                .parse::<chrono_tz::Tz>()
                .map_err(|_| DrugClawError::Config(format!("Invalid timezone: {tz_raw}")))?;
...
Use operator password + API keys for Web auth."
        ...
        for price in &mut self.model_prices {
            ...
            if price.model.is_empty() {
                return Err(DrugClawError::Config(
        ...
        if self.api_key.is_empty() && !provider_allows_empty_api_key(&self.llm_provider) {
            return Err(DrugClawError::Config("api_key is required".into()));
        }
        if is_openai_codex_provider(&self.llm_provider) {
            if !self.api_key.trim().is_empty() {
                return Err(DrugClawError::Config(
                    "openai-codex ignores drugclaw.config.yaml api_key. Configure ~/.codex/auth.json or run `codex login` instead.".into(),
                ));
            }
            if self
                .llm_base_url
            ...
            {
                return Err(DrugClawError::Config(
                    "openai-codex ignores drugclaw.config.yaml llm_base_url. Configure ~/.codex/config.toml instead.".into(),
                ));
            }
    ...
    #[allow(dead_code)]
    pub fn save_yaml(&self, path: &str) -> Result<(), DrugClawError> {
        let content = serde_yaml::to_string(self)
            .map_err(|e| DrugClawError::Config(format!("Failed to serialize config: {e}")))?;
...
"#;
    ...
    fn test_config_default_values() {
        ...
        assert!(config.data_dir.ends_with(".drugclaw"));
        assert!(std::path::PathBuf::from(&config.working_dir)
            .ends_with(std::path::Path::new(".drugclaw").join("working_dir")));
    ...
    #[test]
    fn test_default_data_dir_uses_drugclaw_home() {
        let yaml = "telegram_bot_token: tok\nbot_username: bot\napi_key: key\n";
        let config: Config = serde_yaml::from_str(yaml).unwrap();
        assert!(config.data_dir.ends_with(".drugclaw"));
    }
    ...
    fn test_config_sandbox_defaults_to_off() {
        ...
        assert!(!config.sandbox.no_network);
        assert!(config.sandbox.require_runtime);
        assert_eq!(config.sandbox.image, DEFAULT_SANDBOX_IMAGE);
    }
    ...
    fn test_runtime_and_skills_dirs_from_root_data_dir() {
        let mut config = test_config();
        config.data_dir = "./drugclaw.data".into();

        let runtime = std::path::PathBuf::from(config.runtime_data_dir());
        let skills = std::path::PathBuf::from(config.skills_data_dir());

        assert!(runtime.ends_with(std::path::Path::new("drugclaw.data").join("runtime")));
        assert!(skills.ends_with(std::path::Path::new("drugclaw.data").join("skills")));
    }
    ...
    fn test_skills_dir_uses_config_override() {
        let mut config = test_config();
        config.skills_dir = Some("./drugclaw.data/skills".to_string());
        let skills = std::path::PathBuf::from(config.skills_data_dir());
        assert!(skills.ends_with(std::path::Path::new("drugclaw.data").join("skills")));
    }
...
channels:
  matrix:
    ...
    bot_user_id: "@drugclaw:example.com"
"##;
        let mut config: Config = serde_yaml::from_str(yaml).unwrap();
        config.post_deserialize().unwrap();
    ...
    fn test_post_deserialize_openai_codex_rejects_plain_api_key_without_oauth() {
        ...
        assert!(msg.contains("ignores drugclaw.config.yaml api_key"));
    }
...
"#;
    ...
    fn test_post_deserialize_normalizes_a2a_config() {
        ...
        config.a2a.public_base_url = Some(" https://drugclaw.example.com/ ".into());
        ...
        config.a2a.peers.insert(
            ...
            A2APeerConfig {
                ...
                description: Some(" worker ".into()),
                default_session_key: Some("  ".into()),
            },
        );
        config.a2a.peers.insert(
            " ".into(),
        ...
        assert_eq!(
            ...
            Some("https://drugclaw.example.com")
        );
        ...
        assert_eq!(peer.description.as_deref(), Some("worker"));
        assert!(peer.default_session_key.is_none());
        assert_eq!(config.a2a.peers.len(), 1);
    }
...
"#;
    ...
    fn test_post_deserialize_expands_paths() {
        ...
        assert_eq!(PathBuf::from(&config.data_dir), home.join(".drugclaw"));
