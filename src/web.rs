...
fn config_path_for_save() -> Result<PathBuf, (StatusCode, String)> {
    match Config::resolve_config_path() {
        ...
        Ok(None) => Ok(PathBuf::from("./drugclaw.config.yaml")),
...
fn hook_token_from_headers(headers: &HeaderMap) -> Option<String> {
    auth_token_from_headers(headers).or_else(|| {
        headers
            ...
            .or_else(|| headers.get("x-drugclaw-hook-token"))
...
mod tests {
    ...
    use crate::{error::DrugClawError, llm_types::ResponseContentBlock};
    use axum::body::Body;
    ...
    impl LlmProvider for DummyLlm {
        async fn send_message(
            &self,
            _system: &str,
            _messages: Vec<drugclaw_core::llm_types::Message>,
            _tools: Option<Vec<drugclaw_core::llm_types::ToolDefinition>>,
        ) -> Result<drugclaw_core::llm_types::MessagesResponse, drugclaw_core::error::DrugClawError>
        {
            Ok(drugclaw_core::llm_types::MessagesResponse {
                content: vec![drugclaw_core::llm_types::ResponseContentBlock::Text {
    ...
    impl LlmProvider for SlowLlm {
        async fn send_message(
            ...
            _messages: Vec<drugclaw_core::llm_types::Message>,
            _tools: Option<Vec<drugclaw_core::llm_types::ToolDefinition>>,
        ) -> Result<drugclaw_core::llm_types::MessagesResponse, DrugClawError> {
    ...
    impl LlmProvider for ToolFlowLlm {
        async fn send_message(
            ...
            if n == 0 {
                return Ok(drugclaw_core::llm_types::MessagesResponse {
    ...
    fn test_state_with_config(llm: Box<dyn LlmProvider>, mut cfg: Config) -> Arc<AppState> {
        let dir = std::env::temp_dir().join(format!("drugclaw_webtest_{}", uuid::Uuid::new_v4()));
    ...
    async fn test_web_clear_slash_keeps_scheduled_tasks() {
        ...
        call_blocking(db, move |d| {
            ...
            Ok::<(), drugclaw_core::error::DrugClawError>(())
        })
        .await
        .unwrap();
        ...
        let (session, messages, tasks_len) = call_blocking(db, move |d| {
            Ok::<
                ...
                drugclaw_core::error::DrugClawError,
            >((
                d.load_session(4242)?,
    ...
    async fn test_web_send_plugin_slash_command() {
        let mut cfg = test_config_template();
        let plugin_dir =
            std::env::temp_dir().join(format!("drugclaw_web_plugin_{}", uuid::Uuid::new_v4()));
...
"#,
    ...
    async fn test_a2a_agent_card_route_returns_configured_metadata() {
        ...
        cfg.a2a.public_base_url = Some("https://drugclaw.example.com".into());
        ...
        assert_eq!(
            ...
            Some("https://drugclaw.example.com/api/a2a/message")
        );
    }
